$(function() {
        ...
        layer.confirm('确认删除?', { icon: 3, title: '提示' }, function(index) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    //删完数据后 没有数据后 页码值不会跳上一页 -1
                    //当数据删除完成后，需要判断当前这一页中，是否还有剩余的数据
                    //如果没有剩余的数据了，则让页码值 -1 之后，
                    //再重新调用 initTable方法
                    //4-0
                    if (len === 1) {
                        // 4-2 如果len 的值等于1 证明删除完毕之后，页面上就没有任何数据了
                        //页码值最小必须是1 
    ...
    //通过代理的方式 为 编辑按钮 绑定点击事件处理函数
    $('tbody').on('click', '.btn-edit', function(e) {
        var id = $(this).attr('data-id')
        console.log(id);
        q.cate_id = id
        var state = $(this).attr('data-id').state
        q.state = state
        location.href = '/article/art_pub.html?id=${q.cate_id}'
